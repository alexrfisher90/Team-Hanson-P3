...
async function updateLeaderboard() {
  ...
  for (let i = 0; i < topFive.length; i++) {
    const [score, name] = topFive[i];
    const scoreEl = document.querySelector(`#${i + 1}`);
    const nameEl = document.querySelector(`#${i + 1}name`);
...
async function checkLeaderboard() {
  ...
  if (checkHighScore < localStorage.getItem('fourth') && checkHighScore > localStorage.getItem('fifth')) {
    ...
    const highScores = await updateHighScores(newHighScore);
    ...
    updateScoreNames();
  } else if (checkHighScore < localStorage.getItem('first') && checkHighScore < localStorage.getItem('second') && checkHighScore < localStorage.getItem('third') && checkHighScore > localStorage.getItem('fifth')) {
